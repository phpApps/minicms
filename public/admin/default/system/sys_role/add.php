...
<form method="post">
  <table cellspacing="0" cellpadding="0" width="50%" class="formBox" align="left">
    <tbody>
      <tr>
        ...
        <td><input type="text" name="role_name" value="<?php echo set_value('role_name');?>" class="ws50" />
          <?php echo form_error('role_name');?></td>
      </tr>
      <tr>
        <td>备注</td>
        <td><textarea name="role_remark" class="ws50"><?php echo set_value('role_remark');?></textarea>
  ...
  <table cellspacing="0" cellpadding="0" width="50%" class="formBox" align="right">
    <tbody>
      <tr>
        <td id="permissions"><?php foreach($list_option as $i=> $one):?>
          <fieldset class="mb10">
            <legend align="center">
            <input id="sys_<?php echo $i;?>" name="role_option[]" value="<?php echo $one['option_id'];?>"  type="checkbox"/>
            <?php echo $one['option_name']?></legend>
            <?php foreach($one['children'] as $j=>$two):?>
            <?php if(empty($two['children'])):?>
            <span class="dib mr10">
            <input class="sys_<?php echo $i;?> sys_<?php echo $i.'_'.$j;?>" name="role_option[]" value="<?php echo $two['option_id'];?>"  type="checkbox">
            <?php echo $two['option_name'];?></span>
            <?php else:?>
            <fieldset class="mb10">
              <legend class="fwb">
              <input id="sys_<?php echo $i."_".$j;?>" class="sys_<?php echo $i;?>" name="role_option[]" value="<?php echo $two['option_id'];?>"  type="checkbox"/>
              <?php echo $two['option_name'];?></legend>
              <?php if(!empty($two['children'])):?>
              <?php foreach($two['children'] as $h=>$three):?>
              <span class="dib mr10">
              <input class="sys_<?php echo $i;?> sys_<?php echo $i.'_'.$j;?>" name="role_option[]" value="<?php echo $three['option_id'];?>"  type="checkbox">
