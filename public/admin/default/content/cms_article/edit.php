...
<div class="toolBox">
  <button type="button" onclick="location.href='<?php echo site_url("cms_article/index/$sign")?>'">返回</button>
</div>
...
<form method="post">
    ...
    <tbody>
      <tr>
        ...
        <td><select name="art_enable" class="ws50">
            <option value="1" <?php echo set_select('art_enable',1,$row['art_enable']==1?TRUE:FALSE);?>>是</option>
            <option value="0" <?php echo set_select('art_enable',0,$row['art_enable']==0?TRUE:FALSE);?>>否</option>
          </select>
      ...
      <tr>
        ...
        <td><select name="art_hastop" class="ws50">
            <option value="0" <?php echo set_select('art_hastop',0,$row['art_hastop']==0?TRUE:FALSE);?>>否</option>
            <option value="1" <?php echo set_select('art_hastop',1,$row['art_hastop']==1?TRUE:FALSE);?>>是</option>
          </select>
          <?php echo form_error('art_enable');?></td>
      </tr>
      <tr>
        ...
        <td><select name="art_mid" class="ws50">
            <option value=""></option>
            <?php echo $this->cms_menu_model->menu_option(set_value('art_mid',$row['art_mid']));?>
          </select>
          <?php echo form_error('art_mid');?></td>
      </tr>
      <tr>
        <td>附件图片</td>
        <td><input type="text" id="art_attach" name="art_attach" value="<?php echo set_value('art_attach',$row['art_attach']); ?>" class="ws50" readonly="readonly" />
          <input type="button" id="uploadButton" value="Upload" />
          <?php echo form_error('art_attach');?></td>
      </tr>
      <tr>
        <td>附件图片</td>
        <td><input type="text" id="art_attach2" name="art_attach2" value="<?php echo set_value('art_attach2',$row['art_attach2']); ?>" class="ws50" readonly="readonly" />
          <input type="button" id="uploadButton2" value="Upload" />
          <?php echo form_error('art_attach2');?></td>
      </tr>
      <tr>
        <td>附件文件</td>
        <td><input type="text" id="art_attach3" name="art_attach3" value="<?php echo set_value('art_attach3',$row['art_attach3']); ?>" class="ws50" readonly="readonly" />
          <input type="button" id="uploadButton3" value="Upload" />
          <?php echo form_error('art_attach3');?></td>
      </tr>
      <tr>
        <td>Meta标题<span class="red">*</span></td>
        <td><input type="text" name="art_title" value="<?php echo set_value('art_title',$row['art_title']); ?>" class="ws50"/>
          <?php echo form_error('art_title');?></td>
      </tr>
      <tr>
        <td>Meta关键字</td>
        <td><input type="text" name="art_keyword" value="<?php echo set_value('art_keyword',$row['art_keyword']); ?>" class="ws50"/>
          <?php echo form_error('art_keyword');?></td>
      </tr>
      <tr>
        <td>Meta描述</td>
        <td><textarea name="art_description" class="ws50"><?php echo set_value('art_description',$row['art_description']); ?></textarea>
          <?php echo form_error('art_description'); ?></td>
      </tr>
      <tr>
        <td>文章内容<span class="red">*</span></td>
        <td><textarea name="art_content" style="width:100%"><?php echo set_value('art_content',$row['art_content']); ?></textarea>
  ...
  <table width="40%" cellspacing="0" class="formBox" align="right">
    <tbody>
      <tr>
        <td><?php $lbid = explode(',',$row['art_lbid']);?>
          ...
          <fieldset>
            ...
            <input type="checkbox" name="art_lbid[]" value="<?php echo $label['label_id'];?>" <?php echo set_checkbox('art_lbid[]',$label['label_id'],in_array($label['label_id'],$lbid)?TRUE:FALSE);?> />
